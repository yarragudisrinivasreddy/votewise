...
def ask():
    ...
    translate_svc = current_app.config["TRANSLATE"]
    nlp_svc = current_app.config["NLP"]
    english_question = _resolve_english_question(question, translate_svc)

    # Analyse sentiment to inform response empathy (Cloud Natural Language API).
    sentiment = nlp_svc.analyse_sentiment(english_question)
    logger.info("Sentiment", extra={"label": sentiment.label, "score": sentiment.score})

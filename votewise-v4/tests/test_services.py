...
class TestNaturalLanguageService:
    @patch("app.services.nlp.language_v1")
    def test_analyse_sentiment_returns_result(self, mock_lang):
        mock_client = MagicMock()
        mock_lang.LanguageServiceClient.return_value = mock_client
        mock_lang.Document = MagicMock()
        mock_lang.Document.Type.PLAIN_TEXT = "PLAIN_TEXT"
        ...
        from app.services.nlp import NaturalLanguageService
        svc = NaturalLanguageService("test-project")
        result = svc.analyse_sentiment("I am excited to vote!")
        assert result.score == 0.8
        assert result.magnitude == 1.2

    @patch("app.services.nlp.language_v1")
    def test_sentiment_label_positive(self, mock_lang):
        from app.services.nlp import SentimentResult
        result = SentimentResult(score=0.8, magnitude=1.0)
        assert result.label == "positive"

    @patch("app.services.nlp.language_v1")
    def test_sentiment_label_negative(self, mock_lang):
        from app.services.nlp import SentimentResult
        result = SentimentResult(score=-0.5, magnitude=0.8)
        assert result.label == "negative"

    @patch("app.services.nlp.language_v1")
    def test_sentiment_label_neutral(self, mock_lang):
        from app.services.nlp import SentimentResult
        result = SentimentResult(score=0.1, magnitude=0.2)
        assert result.label == "neutral"

    @patch("app.services.nlp.language_v1")
    def test_analyse_sentiment_graceful_on_failure(self, mock_lang):
        mock_client = MagicMock()
        mock_lang.LanguageServiceClient.return_value = mock_client
        mock_lang.Document = MagicMock()
        mock_lang.Document.Type.PLAIN_TEXT = "PLAIN_TEXT"
        mock_client.analyze_sentiment.side_effect = Exception("quota exceeded")

        from app.services.nlp import NaturalLanguageService
        svc = NaturalLanguageService("test-project")
        result = svc.analyse_sentiment("test text")
        # Should return neutral on failure, not raise
        assert result.score == 0.0
        assert result.label == "neutral"

    @patch("app.services.nlp.language_v1")
    def test_sentiment_to_dict(self, mock_lang):
        from app.services.nlp import SentimentResult
        result = SentimentResult(score=0.5, magnitude=0.9)
        d = result.to_dict()
        assert "score" in d

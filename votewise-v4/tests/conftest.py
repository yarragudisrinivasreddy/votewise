...
def mock_nlp():
    ...
    svc = MagicMock()
    svc.analyse_sentiment.return_value = SentimentResult(score=0.1, magnitude=0.5)
    svc.health.return_value = ServiceHealth(name="natural_language", healthy=True)
    return svc


@pytest.fixture()
def app(mock_translate, mock_gemini, mock_store, mock_secret_manager, mock_storage, mock_nlp):
    ...
    with patch.dict("os.environ", {"GOOGLE_CLOUD_PROJECT": "test-project"}):
        with (
            ...
            patch("app.services.nlp.NaturalLanguageService", return_value=mock_nlp),
            patch("vertexai.init"),
        ):
